...
//float newest_zoffset = zprobe_zoffset;
...
static void report_current_position_G28();
...
static void setup_for_endstop_or_probe_move_G28() {
  #if ENABLED(DEBUG_LEVELING_FEATURE)
    if (DEBUGGING(LEVELING)) DEBUG_POS("setup_for_endstop_or_probe_move", current_position);
    report_current_position_G28();
  #endif
...
static void clean_up_after_endstop_or_probe_move() {
 ...
 static float run_z_probe_mine() {
    ...
    refresh_cmd_timeout();
      ...
      // move down quickly before doing the slow probe
      float z = LOGICAL_Z_POSITION(Z_CLEARANCE_BETWEEN_PROBES);
      if (zprobe_zoffset < 0) z -= current_position[Z_AXIS];
      if (z < current_position[Z_AXIS])
        do_blocking_move_to_z(z, MMM_TO_MMS(Z_PROBE_SPEED_FAST));
  ...
  static float probe_pt(const float &x, const float &y, bool stow = true, int verbose_level = 1) {
    ...
    float old_feedrate_mm_s = feedrate_mm_s;
      ...
      if (current_position[Z_AXIS] > delta_clip_start_height)
        ...
        static float lala =  current_position[Z_AXIS];
    ...
    feedrate_mm_s = old_feedrate_mm_s;
       SERIAL_PROTOCOLPGM(" dfsaf: ");
       SERIAL_PROTOCOL_F(lala, 3);
    return measured_z;
  }

    static float probe_pt_mine(const float &x, const float &y, bool stow = true, int verbose_level = 1) {
    #if ENABLED(DEBUG_LEVELING_FEATURE)
      if (DEBUGGING(LEVELING)) {
        SERIAL_ECHOPAIR(">>> probe_pt(", x);
        SERIAL_ECHOPAIR(", ", y);
    ...
    float measured_z = run_z_probe_mine();
  ...
  static void print_bilinear_leveling_grid() {
    ...
    SERIAL_EOL;
    //bed_level_grid[0][1] = bed_level_grid[0][2];
  }
...
inline void gcode_G28() {
  ...
  setup_for_endstop_or_probe_move_G28();
  #if ENABLED(DEBUG_LEVELING_FEATURE)
 ...
 // report_current_position_G28();
  ...
  if(LevelBedFlag)  {LevelBedFlag=false;do_blocking_move_to_z(delta_clip_start_height);}
  #endif
      ...
      for (uint8_t PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END; PR_OUTER_VAR++) {
        ...
        for (int8_t PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; PR_INNER_VAR += inInc) {
          ...
          if (measured_z == NAN) {
            ...
            bed_level_grid[xCount][yCount] = measured_z + zprobe_zoffset;

          #endif
          
      SERIAL_PROTOCOLPGM("G29 new Z_offset:");
  ...
  inline void gcode_G30() {
    ...
    SERIAL_PROTOCOLLN(measured_z + 0.0001);
        SERIAL_PROTOCOLPGM(" level_bed_v: ");
    SERIAL_PROTOCOLLN(level_bed_value + 0.0001);
        SERIAL_PROTOCOLPGM(" Z_MAX_POS: ");
    SERIAL_PROTOCOLLN(Z_MAX_POS + 0.0001);

  aaaa = (0-(level_bed_value - Z_MAX_POS + measured_z +0.0001));

    SERIAL_PROTOCOLPGM("Please set Z_offset:");
    SERIAL_PROTOCOLLN(aaaa);

// float newest_zoffset = aaaa;
  
    //  zprobe_zoffset = aaaa;

      SERIAL_PROTOCOLPGM("G30 new Z_offset:");
      SERIAL_PROTOCOLLN(zprobe_zoffset);

    //enqueue_and_echo_commands_P(PSTR("M851 Z aaaa"));
    clean_up_after_endstop_or_probe_move();
    ...
    /**< X:0.00 Y:0.00 Z:285.93 E:0.00 Count X: 40168 Y:40168 Z:40168
< X:0.00 Y:0.00 Z:285.93 E:0.00 Count X: 40168 Y:40168 Z:40168
...
inline void gcode_G33() { 
 ...
 inline void gcode_G35(){
    ...
    //  SERIAL_PROTOCOLPGM("new Z_offset:");
    //  SERIAL_PROTOCOLLN(newest_zoffset);
  ...
  a++;
   ...
   if (a == 1) {
      enqueue_and_echo_commands_P(PSTR("G29"));
        a++;
                         static float Y_6 = bed_level_grid[0][6];
      bed_level_grid[0][0] = Y_6+0.3;
      bed_level_grid[0][1] = Y_6+0.3;
        static float III = bed_level_grid[7][0];
        bed_level_grid[8][0] = III+0.3;
        static float X_7_Y_1 = bed_level_grid[7][1];
        bed_level_grid[8][1] = X_7_Y_1+0.3;
        static float X_7_Y_2 = bed_level_grid[7][2];
        bed_level_grid[8][2] = X_7_Y_2+0.3;
        static float X_7_Y_3 = bed_level_grid[7][3];
        bed_level_grid[8][3] = X_7_Y_3+0.3;
        static float X_7_Y_6 = bed_level_grid[7][6];
        bed_level_grid[8][6] = X_7_Y_6+0.3;
        static float X_7_Y_7 = bed_level_grid[7][7];
        bed_level_grid[8][7] = X_7_Y_7+0.3;
        static float X_6_Y_8 = bed_level_grid[6][8];
        bed_level_grid[7][8] = X_6_Y_8+0.3; 
        bed_level_grid[8][8] = X_6_Y_8+0.6;       
  }
    if (a == 2) {
      enqueue_and_echo_commands_P(PSTR("G28"));
      enqueue_and_echo_commands_P(PSTR("G28"));
 ...
 if (a == 0) {
  ...
  a++;
   ...
   if (a == 1) {
    ...
    enqueue_and_echo_commands_P(PSTR("M888"));
...
static char temp=0;
 ...
 { 
   temp++;
   if(temp==1){//print_job_timer.pause();
 ...
 */
inline void gcode_M77() {SERIAL_PROTOCOLPGM("Please set Z_offset:");
     print_job_timer.stop(); }
...
//  static float vvvv = current_position[Z_AXIS];

//  level_bed_value = vvvv
...
static void report_current_position_G28() {
 ...
 static float vvvv = current_position[Z_AXIS];
  ...
  SERIAL_PROTOCOLPGM(" level_bed_value:");
  SERIAL_PROTOCOL(level_bed_value);
  stepper.report_positions();
 ...
 */
 //static float ttt = zprobe_zoffset;
...
inline void gcode_M857() {
 // float value6 = zprobe_zoffset_test;
 SaveABLdataFlag=1;
  if (code_seen('A'))
  {
       for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
...
/*
static float zprobe_zoffset_test = zprobe_zoffset;


inline void gcode_M857() {
   
      zprobe_zoffset_test = zprobe_zoffset-0.1;
         float value6 = zprobe_zoffset_test;
     // SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
             ...
             bed_level_virt_interpolate();
           #endif        
       zprobe_zoffset = value6;
      
      // Config_StoreSettings();
    }
 */

inline void gcode_M856() {
   
  //   zprobe_zoffset_test = zprobe_zoffset-1;
 //      float value5 = zprobe_zoffset_test;
     // SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
               bed_level_grid[x][y] -= 1;
           #if ENABLED(ABL_BILINEAR_SUBDIVISION)
             bed_level_virt_prepare();
             bed_level_virt_interpolate();
           #endif        
 //       zprobe_zoffset = value5;
      
    //   Config_StoreSettings();
    }


inline void gcode_M855() {
   
//      zprobe_zoffset_test = zprobe_zoffset-10;
  //      float value4 = zprobe_zoffset_test;
   //   SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
               bed_level_grid[x][y] -= 10;
           #if ENABLED(ABL_BILINEAR_SUBDIVISION)
             bed_level_virt_prepare();
             bed_level_virt_interpolate();
           #endif        
   //     zprobe_zoffset = value4;
      
 //      Config_StoreSettings();
    }


inline void gcode_M854() {
   
    //  zprobe_zoffset_test = zprobe_zoffset+0.1;
     //   float value3 = zprobe_zoffset_test;
    //  SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
               bed_level_grid[x][y] += 0.1;
...
inline void gcode_M853() {
  ...
  //       zprobe_zoffset_test = zprobe_zoffset+1;
  //     float value2 = zprobe_zoffset_test;
   //   float value2 = ttt+1;
    //  SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
...
inline void gcode_M852() {
   ...
   //   zprobe_zoffset_test = zprobe_zoffset+10;
       // float value1 = zprobe_zoffset_test;
//      SaveABLdataFlag=1;
            for (uint8_t x = 0; x < ABL_GRID_MAX_POINTS_X; x++)
             for (uint8_t y = 0; y < ABL_GRID_MAX_POINTS_Y; y++)
               bed_level_grid[x][y] += 10;
...
void process_next_command() {
  ...
  switch (command_code) {
      ...
      case 35: 
       ...
       case 36: 
        ...
        case 29: // G29 Detailed Z probe, probes the bed at 3 or more points.
          LevelBedFlag=true;
          gcode_G29();
          SaveABLdataFlag=true; 
       
          bed_level_grid[0][1]=bed_level_grid[0][2]+(bed_level_grid[0][2]-bed_level_grid[0][3]);
          bed_level_grid[0][0]=bed_level_grid[0][1]+(bed_level_grid[0][1]-bed_level_grid[0][2]);
          bed_level_grid[0][7]=bed_level_grid[0][6]+(bed_level_grid[0][6]-bed_level_grid[0][5]);
          bed_level_grid[0][8]=bed_level_grid[0][7]+(bed_level_grid[0][7]-bed_level_grid[0][6]);

          bed_level_grid[1][0]=bed_level_grid[1][1]+(bed_level_grid[1][1]-bed_level_grid[1][2]);
          bed_level_grid[1][8]=bed_level_grid[1][7]+(bed_level_grid[1][7]-bed_level_grid[1][6]);

          bed_level_grid[7][0]=bed_level_grid[7][1]+(bed_level_grid[7][1]-bed_level_grid[7][2]);
          bed_level_grid[1][8]=bed_level_grid[1][7]+(bed_level_grid[1][7]-bed_level_grid[1][6]);

          bed_level_grid[8][1]=bed_level_grid[8][2]+(bed_level_grid[8][2]-bed_level_grid[8][3]);
          bed_level_grid[8][0]=bed_level_grid[8][1]+(bed_level_grid[8][1]-bed_level_grid[8][2]);
          bed_level_grid[8][7]=bed_level_grid[8][6]+(bed_level_grid[8][6]-bed_level_grid[8][5]);
          bed_level_grid[8][8]=bed_level_grid[8][7]+(bed_level_grid[8][7]-bed_level_grid[8][6]);          
    ...
    case 'M': switch (codenum) {
      ...
      case 888: // M999: Restart after being Stopped
        gcode_M888();
        break;
